 ...
 */
const getFormat = (formatType, ratingLimit, ageLimitType, ageLimit) => {
  ...
  switch (formatType) {
    case 'openSingle':
      return { ...baseFormat, type: 'openSingle' }
    case 'ratedSingle':
      if (!ratingLimit) throwError('Rating limit is required for rated events')
      return { ...baseFormat, type: 'ratedSingle', ratingLimit }
    case 'ageSingle':
      if (!ageLimitType) throwError('Age limit type (U/O) is required for age events')
      if (!ageLimit) throwError('Age limit is required for age events')
      return { ...baseFormat, type: 'ageSingle', ageLimitType, ageLimit }
    default:
      throwError(`Unknown format type: ${formatType}`)
  }
}
...
export const createTournament = async (body) => {
  validateCreateTournamentInput(body)

  const { name, date, formatType, maxParticipants = 0, ratingLimit, ageLimitType, ageLimit } = body

  const db = getDB()
  const collection = db.collection(COLLECTION)

  // Check for duplicate
  const existing = await collection.findOne({ name, date })
  if (existing) {
    throwError('A tournament with the same name and date already exists')
  }

  const format = getFormat(formatType, ratingLimit, ageLimitType, ageLimit)
  ...
  const tournament = {
    ...
    maxParticipants,
    participants: [],
    stages: format.stages.map((stageType) => {
      if (stageType === 'group') {
        return {
          type: 'group',
          config: { advancingCount: 2 },
          groups: [],
...
const validateCreateTournamentInput = (body) => {
  if (!body) throwError('Request body is required')
  if (!body.name) throwError('Tournament name is required')
  if (!body.date) throwError('Tournament date is required')
  if (!body.formatType) throwError('Format type is required')
}
