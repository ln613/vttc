...
- **League rounds fetched on their own broadcast, not chained to the event** (2026-09-14) — `fetchEvent` was calling `leagueActions.load()`, so every event refetch also pulled a full `getLeague` (every round, every match, 22 KB) for data that mostly hadn't changed. The league now has its own jittered, de-duplicated subscription like every other live view. One Enter Score on a league week: **76 requests → 11**.
- **`events&full=true` scoped to today** (2026-09-14) — full mode had no date filter, so it read *every* event the club has ever run, in full, and discarded almost all of it client-side. The Schedule page draws an event only when it (or a series sibling) has a match on a table or in the queue, and that queue is built from `getStartedEvents()`, which is already today-only — so a non-today event could never be drawn. One line, `query.date = getClubDate()` in full mode only: **24 docs / 1,167 KB / 12,097 ms → 1 doc / 17 KB / 209 ms (98%)**. Summary mode is untouched, because the events list is a history and is already cheap via its projection. This is the same shape as the `getLiveScore` bill — fetch everything, keep a little — one endpoint behind the fix.
...
### 6. Send fewer fields before shortening ids
Measured 2026-09-14 against the deployed site, after someone asked what
integer ids would save over ObjectId/`generateId()` strings. Replacing every
id in a real response with a sequential integer:

| Endpoint | Raw JSON | gzip | **brotli (what bandwidth bills)** |
| --- | --- | --- | --- |
| `type=events` (108 KB) | 29.3% | 34.9% | **37.1%** |
| `type=liveScore` (8 KB) | 18.6% | 14.0% | **17.1%** |
| `type=event` (140 KB) | 15.2% | 10.6% | **8.6%** |

The spread is repetition, not size: `type=event` has 874 id references but
only **143 distinct**, so the compressor already collapses them and integers
add almost nothing; `type=events` has 393 distinct in 1,375, so the ids are
the incompressible part and integers win.

Two things that decide whether this is worth doing:

- **The raw column is the one that matters.** At ~95 KB/s, read time is
  bytes *stored*, so smaller ids buy 15–29% off every query and therefore off
  compute — which outweighs the wire saving (bandwidth is 20 credits/GB,
  compute was ~250 credits of the 2026-09-07 bill).
- **67–76% of id text is Mongo `ObjectId`.** Capturing that share means
  abandoning document `_id` as the id, which the project's own standard
  requires, plus migrating existing data and giving up collision-free
  generation. The remaining **24–33% is `generateId()`**
  (`"1757894400000-k3j2h1g9f"`, 25 bytes on the wire) for participants,
  matches, games and sub-matches — ours, no rule in the way.

**Do the cheaper thing first: send fewer fields.** `type=event` ships 140 KB
and the page reads a fraction of it; a projection in the shape of
`EVENT_TRIAGE_PROJECTION` or the `getRevenue` fix costs far less than an id
migration and removes whole fields rather than trimming bytes off one. Note
that compression only helps bandwidth — a field not sent is also a field not
read, which is where the compute goes.

- **Gain:** unquantified, but the two comparable projections so far were 95%
  (`getRevenue`) and 96% (`getStartedEvents`).
- **Why deferred:** nothing is hurting right now — the date filter above took
  98% off the endpoint that was.
- **Effort/risk:** low / low for a projection; high / high for the id change,
  which should stay shelved unless `generateId()` alone is worth a third of
  the id cost.

### 7. Three `live-score` broadcasts per `finishMatch`, not one
Measured 2026-09-14 on `netlify dev`. One Enter Score on a league sub-match
now costs **8 requests** (was 76), and counters in the browser show the
client is behaving exactly as designed:

```
broadcast→eventDetail: 3     fetchEvent:     1   (the direct refetch after the POST)
broadcast→league:      3     fetchLeague:    3
broadcast→liveScore:   3     fetchLiveScore: 3
```

Three subscribers, three broadcasts, one fetch each — no multiplication
anywhere. The whole remaining cost is that **one POST produced three
broadcasts.**

Ruled out, with evidence:

- **The Pusher layer.** A probe that subscribed as a client and fired a
  single `notifyLiveScoreUpdate()` received exactly one broadcast. The
  leading-edge/trailing coalescing in `pusher.js` is not duplicating.
- **The client's refetch machinery.** The counters above are 1:1 per
  broadcast. `createJitteredRefetch` is not multiplying runs.
- **The live-score read path.** `getLiveScore` can only reach
  `notifyLiveScoreUpdate` through `autoGenerateForEvent`, which needs
  `runAutoStart` — and the browser's `liveScore` calls carry no such param.
  Its other notify, `notifyTableAssigned`, is on a per-user channel and is
  disabled (`NOTIFICATIONS_ENABLED`).

So `notifyLiveScoreUpdate` was called three times, and every call site is a
POST handler — but the tab only posted once.

**Most likely: another tab.** `acquireMatchSession` and
`releaseMatchSession` both broadcast, and the Live Score page's admin
heartbeat calls `getLiveScore` *with* `runAutoStart`, which can broadcast
via `autoGenerateForEvent`. Any of those land on this client's channel while
never appearing in its own network log.

**How to confirm, 30 seconds:** close every other tab and device, `__reset()`,
Enter Score, `__counts()`. If the broadcast counters drop to 1, that was it,
and there is nothing to fix. If they stay at 3, instrument
`notifyLiveScoreUpdate` server-side with a stack trace and read the dev
server's terminal.

- **Effort/risk:** trivial to confirm; unknown to fix, and possibly nothing
  to fix.

### 8. Netlify build-ignore rule for VTTC
...
### 9. Spectator freshness without polling
...
**Rejected** — see #9. Cheap in Pusher terms, expensive in Netlify credits
